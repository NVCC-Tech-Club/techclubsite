...
function parseEventRRule(eventProps, dateEnv) {
    ...
    for (let exdateInput of exdateInputs) {
        let res = internal_cjs.parseMarker(exdateInput);
...
function parseRRuleObject(rruleInput, dateEnv) {
    ...
    function processDateInput(dateInput) {
        if (typeof dateInput === 'string') {
            let markerData = internal_cjs.parseMarker(dateInput);
            if (markerData) {
...
function analyzeRRuleString(str) {
    ...
    function processMatch(whole, introPart, datePart) {
        let result = internal_cjs.parseMarker(datePart);
...
const RRULE_EVENT_REFINERS = {
    rrule: internal_cjs.identity,
    exrule: internal_cjs.identity,
    exdate: internal_cjs.identity,
    duration: internal_cjs.createDuration,
};
